...
//        return;
    if (!IsBlockchainSynced() && RequestedMasternodeAssets > MASTERNODE_SYNC_LIST)
        return;
...
//            return;
//        }
        if (pnode->nVersion >= MIN_MNW_PING_VERSION) {
...
//                    if (IsSporkActive(SPORK_2_MASTERNODE_PAYMENT_ENFORCEMENT)) {
